...
class ObjectProxy(object):
    ...
    def __getattr__(self, attr_):
        ...
        def wrapper(*args, **kw):
            ...
            return apply_proxy(self._charpool, attr(*args, **kw), self._charobj)
        return wrapper
...
class StrokeProxy(ObjectProxy):
    ...
    def __getitem__(self, i):
        return apply_proxy(self._charpool, self._obj[i], self._charobj)
...
class WritingProxy(ObjectProxy):
    ...
    # Note: some method calls which are not mentioned below
    # because of call that theses methods make to Stroke and Point objects
...
def apply_proxy(charpool, obj, charobj):
    return _apply_proxy(charpool, obj, charobj)

def _apply_proxy(charpool, obj, charobj, reclevel=0):
    try:
        return OBJ_PROXY[obj.__class__](charpool, obj, charobj)
    except KeyError:
        if (isinstance(obj, tuple) or isinstance(obj, list)) and reclevel <= 1:
            return [_apply_proxy(charpool, ele, charobj, reclevel+1) \
                        for ele in obj]
        else:
            return obj
...
class CharPool(dict):  
    ...
    # We don't want changes to be immediately reflected back in the db
    # for performance reasons. The CharPool keeps track of what object need
    # be updated.
...
WHERE charid=?""", (char.get_utf8(), char.get_writing().get_n_strokes(),
                    adapt_character(char), char.hash(), char.charid))
...
""")
    ...
    def get_character_from_row(self, row):
        ...
        char = convert_character(row['data'])
    ...
    def bind(self, path):
        ...
        self._con.row_factory = dict_factory #sqlite3.Row 
        self._c = self._con.cursor()
        self._charpool = CharPool(self._c)
...
VALUES (?,?,?,?,?)""", (i, character.get_utf8(),
                        ...
                        adapt_character(character), character.hash()))
